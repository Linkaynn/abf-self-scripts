...
;// CONCATENATED MODULE: ./src/domain/WithStandPainControl/WithStandPainControl.script.ts

var getWithStandPainReductionValue = function (value) {
    if (value < 80)
        return 0;
    if (value < 120)
        return -10;
...
var data = getSelectedTokenActorData();
if (data) {
    var withstandPain = data.secondaries.vigor.withstandPain.final.value;
    custom1d100({
        title: 'Control de resistencia al dolor',
        additionalDiceValues: "".concat(withstandPain, "[Resistencia al dolor]"),
    }).then(function (dice) {
        if (dice.total === undefined)
            return;
        var message = getWithStandPainReductionValue(dice.total);
        dice.toMessage({
            speaker: ChatMessage.getSpeaker({ token: getSelectedToken() }),
            flavor: buildCustomMessageDiceFlavor({
                title: 'Reducción de negativos alcanzada',
                message: "".concat(message, " puntos"),
            }),
        });
    });
}
